...
import { getWeeks } from "./DateTable";
...
const monthsOfYear = months.map((monthName: string, index: number) => {
    return {
      ...
      const weeks = getWeeks(context.year, context.month);

      const [selectedMonth, setSelectedMonth] = useState<{id:number, month: string, yearCount: number}>(monthsOfYear[context.month])
      ...
      const handleChangeMonth = (changer : boolean) =>{
          ...
          context?.setMonth(monthsOfYear.findIndex(month => month.id === selectedMonth.id))
        }
                            ...
                            {week.map((day, index) => { 
                                return context.month === day.month ? (
                                  ...
                                  <td className={"current-month"} 
                                    onClick={() => context.setDateString(`${day.date}.${day.month}.${context.year}`)}
                                    key={index}
                                    >
                                    {day.date}
                                  </td>
